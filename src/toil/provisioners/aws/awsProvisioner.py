...
class AWSProvisioner(AbstractProvisioner):
    ...
    def __init__(self, clusterName, zone, nodeStorage, nodeStorageOverride, sseKey):
        super(AWSProvisioner, self).__init__(clusterName, zone, nodeStorage, nodeStorageOverride)
        self.cloud = 'aws'
        self._sseKey = sseKey
        if not zone:
    ...
    def getNodeShape(self, nodeType, preemptable=False):
        ...
        if disk == 0:
            ...
            # the root volume
            disk = (self._nodeStorageOverride.get(nodeType) or self._nodeStorage) * 2 ** 30
    ...
    def addNodes(self, nodeType, numNodes, preemptable, spotBid=None, zoneSubnetOverride=None):
        ...
        instanceType = E2Instances[nodeType]
        bdm = self._getBlockDeviceMapping(instanceType, rootVolSize=self._nodeStorageOverride.get(nodeType) or self._nodeStorage)
